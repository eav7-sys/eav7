...
// ============================================================================
// S5 — raiz_confiavel_do_header
// ============================================================================

/// Header íntegro (assinado de verdade) devolve o `stateRoot` do bloco.
#[test]
fn s5_raiz_confiavel_do_header_apos_integridade() {
    use eav7::block::{block_to_json_line, build_block, BuildParams, STATEROOT_HEIGHT};

    let carteira = carteira();
    let raiz = eav7::eav_hash_one(b"fase-s:state-root");
    let bloco = build_block(
        &carteira,
        BuildParams {
            height: STATEROOT_HEIGHT + 1,
            previous_hash: eav7::GENESIS_PREVIOUS_HASH.to_string(),
            timestamp: 1_700_000_000_000,
            transactions: vec![],
            state_root: Some(raiz.clone()),
            producer_account: None,
            omit_public_keys: false,
        },
    )
    .expect("bloco assinado");
    let corpo: serde_json::Value =
        serde_json::from_str(&block_to_json_line(&bloco).expect("linha")).expect("json");

    let url = no_falso(move |req| {
        assert!(req.caminho.starts_with("/blocks/"));
        (200, corpo.clone())
    });

    let obtida = Eav7Client::novo(&url)
        .raiz_confiavel_do_header(&(STATEROOT_HEIGHT + 1).to_string())
        .expect("raiz");
    assert_eq!(obtida, raiz);
}

/// Hash/assinatura adulterados → [`ErroCliente::ProvaInvalida`], não uma raiz.
#[test]
fn s5_header_adulterado_e_prova_invalida() {
    use eav7::block::{block_to_json_line, build_block, BuildParams, STATEROOT_HEIGHT};
    ...
    let mut bloco = build_block(
        ...
        BuildParams {
            ...
            timestamp: 1_700_000_000_000,
            transactions: vec![],
            state_root: Some(eav7::eav_hash_one(b"fase-s:outra-raiz")),
            producer_account: None,
            omit_public_keys: false,
        },
    )
    .expect("bloco");
    bloco.hash = eav7::eav_hash_one(b"hash-mentiroso");
    let corpo: serde_json::Value =
        serde_json::from_str(&block_to_json_line(&bloco).expect("linha")).expect("json");

    let url = no_falso(move |_| (200, corpo.clone()));
    let erro = Eav7Client::novo(&url)
        .raiz_confiavel_do_header("1200001")
        .expect_err("hash mentiroso");
    assert!(matches!(erro, ErroCliente::ProvaInvalida(_)), "{erro}");
}
