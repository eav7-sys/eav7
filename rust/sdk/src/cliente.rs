//! Cliente HTTP da API do nó EAV7.
//!
...
//!
//! Para fechar o círculo, obtenha a raiz com [`Eav7Client::raiz_confiavel_do_header`]
//! (verifica integridade do bloco) e passe-a a `saldo_provado`.

use std::time::Duration;

use eav7::block::{block_from_json, verify_block_integrity, BlockSigner};
...
pub struct Conta {
    ...
    /// Cota GB · Assinatura Livre (`/address` → `gb`), quando o nó a expõe.
    pub gb: Option<GbCota>,
}

/// Barra GB do dia (bytes ponderados) — plano 12.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GbCota {
    pub max: u64,
    pub available: u64,
...
impl Eav7Client {
    ...
    pub fn validadores_tipados(&self) -> R<Vec<Validador>> {
        ...
        Self::validadores_da_lista(&v, "current")
    }

    /// Standby (banco) de `/validators` → `bank` (plano 17).
    pub fn validadores_banco(&self) -> R<Vec<Validador>> {
        let v = self.validadores()?;
        Self::validadores_da_lista(&v, "bank")
    }

    fn validadores_da_lista(v: &serde_json::Value, chave: &str) -> R<Vec<Validador>> {
        let atuais = v
            .get(chave)
    ...
    pub fn conta(&self, endereco: &str) -> R<Conta> {
        ...
        let unbonding = v
            ...
            .unwrap_or_default();
        let gb = v.get("gb").and_then(|g| {
            Some(GbCota {
                max: g.get("max").and_then(serde_json::Value::as_u64).unwrap_or(0),
                available: g.get("available").and_then(serde_json::Value::as_u64).unwrap_or(0),
                used: g.get("used").and_then(serde_json::Value::as_u64).unwrap_or(0),
            })
        });
        Ok(Conta {
    ...
    /// Bloco por altura ou hash (`GET /blocks/{ref}`).
    pub fn bloco(&self, referencia: &str) -> R<serde_json::Value> {
        self.get(&format!("/blocks/{referencia}"))
    }

    /// `stateRoot` de um header **depois** de [`verify_block_integrity`].
    ///
    /// É a fonte de `raiz_confiavel` para [`Self::saldo_provado`]: o nó pode
    /// mentir no `/proof`, mas não consegue forjar um bloco cujo hash e
    /// assinaturas fechem. Abaixo de `STATEROOT_HEIGHT` a API ainda devolve o
    /// bloco; este método recusa com [`ErroCliente::Resposta`] se não houver raiz.
    pub fn raiz_confiavel_do_header(&self, referencia: &str) -> R<String> {
        let v = self.bloco(referencia)?;
        let texto = serde_json::to_string(&v)
            .map_err(|e| ErroCliente::Resposta(format!("bloco não serializa: {e}")))?;
        let canon = eav7::transaction::parse_json(&texto)
            .map_err(|e| ErroCliente::Resposta(format!("bloco ilegível: {e}")))?;
        let block = block_from_json(&canon)
            .map_err(|e| ErroCliente::Resposta(format!("bloco malformado: {e}")))?;
        verify_block_integrity(&block)
            .map_err(|e| ErroCliente::ProvaInvalida(format!("integridade do header: {e}")))?;
        block.state_root.ok_or_else(|| {
            ErroCliente::Resposta(
                "bloco sem stateRoot (fork dormente ou altura abaixo de STATEROOT_HEIGHT)".into(),
            )
        })
    }
    ...
    /// que minta raiz e saldo de forma coerente. Com ela (via
    /// [`Self::raiz_confiavel_do_header`]), a garantia é completa.
    ...
    /// Aplica `PERMISSION_UPDATE` v2 (virar Âncora) — assinado pela carteira atual.
    ///
    /// Use [`crate::ancora_preparar`] para gerar owners/witness e
    /// [`AncoraPrep::dados_permission_update`] para o `data`.
    pub fn ancora_aplicar_permissoes(&self, dados: JsonValue) -> R<Submissao> {
        self.executar("PERMISSION_UPDATE", 0, move |s| s.com_dados(dados))
    }

    /// Propõe rotação da witness (`PERMISSION_PROPOSE`) — ainda exige aprovações owner.
    ///
    /// `conta` = endereço Âncora; a carteira do cliente deve ser chave **owner**.
    pub fn ancora_propor_rotate_witness(&self, conta: &str, novo_witness: &str) -> R<Submissao> {
        let dados = crate::ancora_dados_rotate_witness(conta, novo_witness);
        self.executar("PERMISSION_PROPOSE", 0, move |s| s.com_dados(dados))
    }
