...
/// O config gerado tem de estar em dia com a referência. Se `src/config.js`
/// mudou e ninguém regerou, este teste acusa antes de a divergência virar
/// comportamento.
#[test]
fn o_config_gerado_esta_em_dia_com_a_referencia() {
    let atual = fs::read_to_string(raiz().join("src/config.rs")).expect("config.rs");
    let saida = std::process::Command::new("node")
        .arg(raiz().parent().unwrap().join("bin/eav7-config-rs.js"))
        .current_dir(raiz().parent().unwrap())
        .output();

    let Ok(saida) = saida else {
        eprintln!("node indisponível — pulando a conferência de atualidade");
        return;
    };
    assert!(saida.status.success(), "o gerador falhou: {}", String::from_utf8_lossy(&saida.stderr));

    let regerado = fs::read_to_string(raiz().join("src/config.rs")).expect("config.rs");
    assert_eq!(
        atual, regerado,
        "`src/config.js` mudou e `config.rs` está velho — rode: node bin/eav7-config-rs.js",
    );
}
