...
use crate::block::{
    block_validator, build_block, verify_block_integrity, Block, BlockSigner, BuildParams,
    STATEROOT_HEIGHT,
};
...
impl Blockchain {
    ...
    fn verificar_lote(mut blocos: Vec<Block>) -> (Vec<BlocoVerificado>, Option<ChainError>) {
        ...
        if nucleos <= 1 || blocos.len() < 8 {
            // Lote pequeno não paga o custo de criar threads.
            for (i, b) in blocos.iter().enumerate() {
                falhas[i] = verify_block_integrity(b).err();
            }
        } else {
