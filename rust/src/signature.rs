...
impl HybridPublicKey {
    ...
    pub fn from_pem(ecdsa_pem: &str, pq_pem: &str) -> Result<Self, SignatureError> {
        let ecdsa_der = pem_para_der(ecdsa_pem).ok_or(SignatureError::ChaveEcdsaInvalida)?;
        let pq_der = pem_para_der(pq_pem).ok_or(SignatureError::ChavePqInvalida)?;
...
/// Extrai o DER de um PEM `-----BEGIN PUBLIC KEY-----`.
///
/// Exige o rótulo `PUBLIC KEY`: aceitar qualquer rótulo deixaria passar uma
/// `PRIVATE KEY` colada por engano no campo errado, e o erro só apareceria na
/// verificação — longe demais da causa.
