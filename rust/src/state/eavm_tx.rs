...
//! `bin/eav7-vectors-eavm.js`), verificado em `rust/tests/eavm_state.rs`.
//!
...
use super::recursos::{
    canonical_tx_bytes, commit_bandwidth, commit_energy, energia_disponivel, peek_bandwidth,
    peek_energy, taxa_de,
};
...
fn aplicar_transfer(state: &mut State, tx: &Tx, height: u64, fee_limit: Amount) -> R<Amount> {
    ...
    // --- daqui para baixo, só escrita (todas as validações passaram) ---
    state.debitar(&tx.from, total)?;
    // `this.credit(tx.to, amount)` — o JS com `to === null` credita a chave
    // "null" (coerção de chave de objeto). Espelhado ao byte: inventar uma
    // rejeição aqui divergiria da referência; o envelope (stateless) garante
    // destino válido no caminho real.
...
fn aplicar_vm(
    ...
    //
    // O gás gasto vira energia: custo total = base + ceil(gasUsed / GAS_PER_ENERGY).
    ...
    // `acct` — energyUsed/energyBlock e bandwidthUsed/bandwidthBlock são consenso.
    commit_energy(acc, height, &energia);
    if let Some(b) = &bw {
        commit_bandwidth(acc, height, b);
    }
