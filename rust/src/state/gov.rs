...
// (`config.rs`, gerado por `bin/eav7-config-rs.js`), e o texto sobreviveu à
...
fn validadores(state: &State, override_stake: Option<(&str, Amount)>) -> R<Vec<String>> {
    ...
    // Peso decrescente; desempate por endereço crescente. Determinístico.
...
fn aplicar_op(state: &mut State, account: &str, op: OpResolvida, height: u64) -> R<()> {
    match op {
...
fn multisig_propose(state: &mut State, tx: &Tx, ctx: &Ctx) -> R<()> {
    ...
    // Usar `perm.keys` direto aqui era o bug: numa conta v2 esse mapa é vazio.
    let permission_id = match d.get("permissionId") {
        None | Some(JsonValue::Null) => 0u64,
        outro => como_u64(outro).ok_or_else(|| erro("permissionId inválido"))?,
    };
    let gasto = nivel_de_gasto(perm, permission_id)
        .ok_or_else(|| erro(format!("permissão active {permission_id} inexistente")))?;
    let peso = *gasto
        .nivel
        .keys
        .get(&tx.from)
    ...
    // ESCOPO: `operations` ausente = tudo liberado.
    ...
    cobrar_taxa(state, tx, ctx)?;
    match resolvida {
        Some(op) => aplicar_op(state, &account, op, ctx.height)?,
        None => {
...
fn multisig_approve(state: &mut State, tx: &Tx, ctx: &Ctx) -> R<()> {
    ...
    match resolvida {
        ...
        Some(op) => {
            ...
            aplicar_op(state, &conta, op, ctx.height)?;
        }
        None => {
...
mod tests {
    ...
    fn ops_v2_nao_existem_abaixo_do_fork() {
        ...
        // A partir do fork v2 a mesma operação passa — é o que destrava o validador
        // multiassinatura.
        aplicar(&mut s, &tx, &ctx(PERMISSIONS_V2_HEIGHT)).unwrap();
        assert_eq!(
