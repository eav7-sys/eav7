...
pub struct BuildParams {
    ...
    pub producer_account: Option<String>,
    /// Acima de `COMPACT_BLOCK_HEIGHT`: omitir pubs no fio (já conhecidas na cadeia).
    pub omit_public_keys: bool,
}
...
pub fn build_block(signer: &dyn BlockSigner, p: BuildParams) -> Result<Block, BlockError> {
    ...
    let compact = p.height >= crate::config::COMPACT_BLOCK_HEIGHT;
    let (public_key, pq_public_key) = if compact && p.omit_public_keys {
        (None, None)
    } else {
        let pk_wire = crate::signature::HybridPublicKey::encode_public_key_wire(
            signer.public_key_pem(),
            compact,
        )
        .map_err(|e| format!("chave pública do produtor inválida: {e}"))?;
        let pq_wire = crate::signature::HybridPublicKey::encode_public_key_wire(
            signer.pq_public_key_pem(),
            compact,
        )
        .map_err(|e| format!("chave pública do produtor inválida: {e}"))?;
        (Some(pk_wire), Some(pq_wire))
    };
...
pub fn verify_block_integrity(block: &Block) -> Result<(), BlockError> {
    verify_block_integrity_ex(block, None)
}

/// Como [`verify_block_integrity`], com chaves do produtor já conhecidas na cadeia
/// (bloco compacto sem `publicKey`/`pqPublicKey` no fio — plano 21 A2).
pub fn verify_block_integrity_ex(
    block: &Block,
    known_keys: Option<(&str, &str)>,
) -> Result<(), BlockError> {
    ...
    let (pk, pqpk) = match (&block.public_key, &block.pq_public_key, known_keys) {
        (Some(a), Some(b), _) => (a.as_str(), b.as_str()),
        (None, None, Some((a, b))) if block.height >= crate::config::COMPACT_BLOCK_HEIGHT => (a, b),
...
mod tests {
    ...
    /// Baseline do plano 21 / T6.2: bloco ocioso com pubs PEM+Base64 ainda é
    ...
    fn tamanho_fio_bloco_ocioso_documentado() {
        ...
        assert!(
            (4_000..12_000).contains(&n_legado),
            "bloco ocioso legado = {n_legado} bytes (esperado ~5–8 KB com PEM)"
        );

        let compacto = bloco(crate::config::COMPACT_BLOCK_HEIGHT);
        let n_compacto = crate::transaction::canonical_json(&block_to_json(&compacto)).len();
        assert!(
            n_compacto < n_legado,
            "SPKI-base64 tem de encolher vs PEM: compacto={n_compacto} legado={n_legado}"
        );
        assert!(
            verify_block_integrity(&compacto).is_ok(),
            "bloco compacto tem de verificar"
        );

        // Omitir pubs (já conhecidas): o ganho grande vs ~0,51 GB/dia ocioso.
        let c = Carteira::nova(1);
        let mut p = params(crate::config::COMPACT_BLOCK_HEIGHT);
        p.omit_public_keys = true;
        let sem_pubs = build_block(&c, p).expect("build omit");
        assert!(sem_pubs.public_key.is_none() && sem_pubs.pq_public_key.is_none());
        let n_omit = crate::transaction::canonical_json(&block_to_json(&sem_pubs)).len();
        assert!(
            n_omit < n_compacto,
            "omitir pubs tem de encolher mais: omit={n_omit} spki={n_compacto}"
        );
        assert!(verify_block_integrity(&sem_pubs).is_err(), "sem known_keys falha");
        assert!(verify_block_integrity_ex(
            &sem_pubs,
            Some((c.public_key_pem(), c.pq_public_key_pem())),
        )
        .is_ok());
    }

    #[test]
    fn chave_compacta_spki_base64_equivale_ao_pem() {
        let c = Carteira::nova(1);
        let wire = crate::signature::HybridPublicKey::encode_public_key_wire(
            c.pq_public_key_pem(),
            true,
        )
        .expect("wire");
        assert!(!wire.contains("BEGIN"), "compacto não é PEM");
        let a = address_from_public_keys(c.public_key_pem(), c.pq_public_key_pem()).unwrap();
        let b = address_from_public_keys(
            &crate::signature::HybridPublicKey::encode_public_key_wire(c.public_key_pem(), true)
                .unwrap(),
            &wire,
        )
        .unwrap();
