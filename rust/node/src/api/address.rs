...
pub fn address_dossier(node: &Node, raw: &str) -> ApiReply {
    ...
    let (g_max, g_avail) = recurso(
        c::GB_DAILY_BYTES,
        c::gb::PER_STAKED_EAV7_BYTES,
        c::gb::REGEN_BLOCKS,
        rstake,
        acc.gb_used,
        acc.gb_block,
...
mod tests {
    ...
    fn dossie_com_saldo_e_stake() {
        ...
        // GB: 1 GB base + 1 MB × 100 EAV7 staked.
        assert_eq!(
            body["gb"]["max"],
            c::GB_DAILY_BYTES + 100 * c::gb::PER_STAKED_EAV7_BYTES
        );
        assert_eq!(body["gb"]["available"], body["gb"]["max"]);
    }
