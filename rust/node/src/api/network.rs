...
pub fn validators_index(node: &Node, now_ms: i64) -> ApiReply {
    ...
    let banco = match eav7::blockchain::validator_bank(st) {
        Ok(v) => v,
        Err(e) => return bad_request(e),
    };
    ...
    let view = |v: &eav7::blockchain::Validator| {
        json!({
            "address": v.address,
    ...
    reply(200, json!({
        ...
        "bankSize": c::VALIDATOR_BANK_SIZE,
        ...
        "current": atuais.iter().map(view).collect::<Vec<_>>(),
        // Standby (plano 17): top após as ativas.
        "bank": banco.iter().map(view).collect::<Vec<_>>(),
...
mod tests {
    ...
    fn validators_com_state_montado_na_mao() {
        ...
        assert_eq!(body["bankSize"], json!(c::VALIDATOR_BANK_SIZE));
