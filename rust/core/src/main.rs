...
Âncora (owners frios + witness no nó):
  eav7-core ancora-init [--dir PATH] [--owners N] [--threshold M] [--force]
            gera N owners (backup) + witness em validator-wallet.json
            NÃO envia tx — só material local + JSON de PERMISSION_UPDATE
...
fn despacha() -> Result<(), String> {
    ...
    match cmd.as_str() {
        ...
        "ancora-init" | "ancora_init" => {
            let a = parse_ancora_init(args)?;
            ancora::executar(a)
        }
...
fn parse_ancora_init(
    mut args: impl Iterator<Item = String>,
) -> Result<ancora::AncoraInitArgs, String> {
    let mut dir = None;
    let mut owners = 3usize;
    let mut threshold = 2u64;
    let mut force = false;
    while let Some(flag) = args.next() {
        let mut valor = || args.next().ok_or_else(|| format!("{flag} exige valor"));
        match flag.as_str() {
            ...
            "--owners" => {
                owners = valor()?
                    .parse()
                    .map_err(|_| "--owners inválido".to_string())?;
            }
            "--threshold" => {
                threshold = valor()?
                    .parse()
            ...
            "--force" => force = true,
            outro => return Err(format!("flag desconhecida em ancora-init: {outro}")),
        }
    }
    Ok(ancora::AncoraInitArgs {
        dir: dir.unwrap_or_else(paths::diretorio_padrao),
        owners,
        threshold,
        force,
