...
| Vetores | Fixtures de conformidade em `vectors/` | — | congelados |

**Total em Rust: ~56k linhas** (ordem de grandeza; medir com `tokei` se precisar).

## Cobertura

| Suíte | Testes |
|---|---:|
| Rust (workspace) | ver `cargo test --workspace` |
| Vetores | em `vectors/` |

## API

Rotas em `rust/node/src/api/*.rs`. Exemplos:
