...
**Fora de escopo nesta fase:** UI gráfica desktop, app mobile,
...
| B2. Candidatar | Após `MIN_VALIDATOR_STAKE`, modo `candidate`; aparece em `/validators` (ou lista de candidatos — se faltar endpoint, criar nos **dois** clientes enquanto protocolo viver) |
...
| Dual protocolo/Rust | Enquanto protocolo for produção, endpoint novo nasce nos dois |
...
- Aposentar protocolo sem Rust validando em produção.
...
| A1 binários oficiais multi-alvo | feito (Linux x64/arm64, macOS arm64, Windows x64; macOS Intel opcional) |
| A4 instaladores / launchd / Windows service | feito (systemd + launchd + doc Windows/NSSM) |
