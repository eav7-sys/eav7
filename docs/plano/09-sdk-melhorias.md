...
| Capacidade | Onde |
|---|---|
| Carteira híbrida (JSON de wallet) | `wallet.rs` |
...
`raiz_confiavel_do_header` (S5) + exemplo `consulta`.
`cargo test -p eav7-sdk` cobre S1–S6 com nó falso. Integração contra nó vivo
(critérios 1 e 4 end-to-end) ainda falta.

## Ligação com a API

Métodos novos do SDK leem/escrevem rotas do nó Rust (`eav7-node` / Core).
