...
Origem do desenho: estrutura própria de níveis (`owner` / `active` / `witness` / `recovery`),
limiar, pesos e escopo de operações. A **recuperação por `recovery`, o timelock e o veto** são
parte do modelo EAV7 v2.
...
1. **Perda de chave é perda total.** Não existe caminho de recuperação. É o modo de falha que
   ...
   Bitcoin existente está perdido. Muitas L1s também não oferecem recuperação nativa.
2. **A chave que assina bloco é a chave que guarda o stake.** O cabeçalho do bloco
...
3. **Validador não pode ter permissões.** A máquina de estado exige `staked == 0` para virar
...
**1. `staked == 0` para virar multisig** (máquina de estado). Sem remover, validador nunca terá
...
**2. Guarda de assinatura única** (máquina de estado). Hoje qualquer conta com permissão é obrigada
...
- **Anti-trava na proposta E na execução.** O estado muda durante o timelock: uma configuração
  ...
  vez de brickar — mesmo padrão do trilho anti-brick da governança (máquina de estado).
...
5. `witness` no consenso de produção de bloco (mudança no consenso / produção de bloco)
...
Escopo de operações como LISTA de tipos nomeados (em vez de bitmap opaco), porque nosso
conjunto é pequeno e clareza vale mais que os bytes economizados num registro raro.
...
- Múltiplas permissões `active` (mantemos 1)
- Nome da permissão (fora de escopo nesta versão)
