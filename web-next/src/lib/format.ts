...
/**
 * Cota GB · Assinatura Livre.
 * A API publica bytes ponderados (`unit: weighted_bytes`); 1 GB = 1e9 bytes
 * (mesma base de `GB_DAILY_BYTES` no nó). Sem isso a UI mostra 11.000.000.000
 * e parece “11 bilhões de GB”.
 */
export function fmtGbBytes(bytes: number | null | undefined): string {
  const v = Math.max(0, Number(bytes ?? 0));
  if (!Number.isFinite(v)) return "—";
  if (v >= 1_000_000_000) {
    const gb = v / 1_000_000_000;
    return (
      gb.toLocaleString(LOCALE, {
        maximumFractionDigits: gb >= 100 ? 1 : 2,
  ...
  if (v >= 1_000) {
    return (v / 1_000).toLocaleString(LOCALE, { maximumFractionDigits: 1 }) + " KB";
  }
  return Math.round(v).toLocaleString(LOCALE) + " B";
}

/** `disponível / teto` da cota diária, já em GB/MB. */
export function fmtGbQuota(available: number, max: number): string {
  return `${fmtGbBytes(available)} / ${fmtGbBytes(max)}`;
}

/** USD no estilo do explorador (`$1,234.56`). */
export function fmtUsd(n: number | null | undefined, digits = 2): string {
  if (n == null || !Number.isFinite(n)) return "—";
  const abs = Math.abs(n);
  const d = abs > 0 && abs < 0.01 ? Math.max(digits, 4) : digits;
  return (
    "$" +
...
/** Nome EAV-NS legível no explorer (`eav7-labs-ancora-1` → `EAV7 Labs Âncora 1`). */
export function fmtNsName(name: string | null | undefined): string {
  if (!name) return "";
