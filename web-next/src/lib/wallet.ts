...
export async function signAndSend(
  ...
  params: {
    ...
    valueWei: bigint;
    chainId: number;
    /** Calldata de contrato (`releaseTo`, etc.). Com data, valueWei pode ser 0. */
    data?: string;
    gasLimit?: bigint;
  }
): Promise<SendResult> {
  const hasData = Boolean(params.data && params.data !== "0x");
  if (!hasData && params.valueWei <= 0n) throw new Error("informe um valor positivo");
  ...
  const raw = await buildSignedTx({
    ...
    gasLimit: params.gasLimit ?? (hasData ? 300_000n : undefined),
  });
