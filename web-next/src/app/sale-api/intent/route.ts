...
export async function POST(req: Request) {
  ...
  if (useRemoteRelayer()) {
    try {
      const res = await fetch(`${relayerBase()}/intent`, {
        method: "POST",
      ...
      return bad(503, "relayer_down", "Sale relayer offline (SALE_RELAYER_URL)");
    }
  }

  try {
    const data = createLocalIntent({ beneficiary0x, rail, usdAmount });
