...
// Cores por tipo de resultado (estilo Tronscan: badge por categoria).
  ...
  return (
    <div ref={boxRef} className={`relative w-full ${className}`} style={{ zIndex: open ? 120 : undefined }}>
      <form
        ...
        className={
          hero
            ? "search-pill flex w-full items-center gap-2 rounded-full bg-panel/80 p-1.5 pl-4 backdrop-blur"
      ...
      >
        <svg width={hero ? 17 : 15} height={hero ? 17 : 15} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={hero ? "search-ico flex-none text-muted transition-colors" : "flex-none text-muted"}>
        ...
        <input
          ...
          autoFocus={autoFocus}
          className={`min-w-0 flex-1 bg-transparent ${hero ? "text-[14px]" : "text-[13px]"} text-ink outline-none placeholder:text-faint`}
        />
        {loading && (
        ...
        <button type="submit" className={hero ? "btn-primary group flex-none !px-5 !py-2 text-[13px]" : "btn-primary flex-none !px-3.5 !py-1.5 text-[12px]"}>
